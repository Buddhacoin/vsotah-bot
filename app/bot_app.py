...
from app.admin_health import build_admin_health_text
...
async def animate_status_dots(message: Message, base_text: str, stop_event: asyncio.Event, interval: float = 0.35):
    """Small Telegram loading animation without touching business logic."""
    frames = ("", ".", "..", "...")
    index = 0
    last_text = None
    while not stop_event.is_set():
        text = f"{base_text}{frames[index % len(frames)]}"
        if text != last_text:
            try:
...
async def admin_handler(message: Message):
    ...
    await message.answer(
        ...
        "/payments — платежи\n"
        "/refstats — партнёрка\n"
        "/health — состояние бота и провайдеров\n"
        "/setplus telegram_id\n"
        "/setpro telegram_id\n"
        "/setvip telegram_id\n"
        "/setfree telegram_id"
...
async def admin_health_command(message: Message):
    ...
    text = await build_admin_health_text(db_pool)
...
async def voice_handler(message: Message):
    ...
    try:
        ...
        if VOICE_REPLY_ENABLED:
            voice_status_message = None
            voice_stop = asyncio.Event()
            voice_task = None
            try:
                voice_status_message = await message.answer("🔊 Голосовой ответ готовится...")
                voice_task = asyncio.create_task(
                    animate_status_dots(voice_status_message, "🔊 Голосовой ответ готовится", voice_stop)
                )
                ...
                if voice_status_message:
                    try:
                        await voice_status_message.delete()
                    except Exception:
                        pass
                ...
                if voice_status_message:
                    try:
                        await voice_status_message.edit_text("⚠️ Не удалось подготовить голосовой ответ.")
                    except Exception:
                        pass
...
async def chat_handler(message: Message):
    ...
    loading_task = asyncio.create_task(animate_status_dots(wait_message, "Печатает ответ", loading_stop))
